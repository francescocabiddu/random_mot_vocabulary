...
Simple effects: children and models are now compared to 10 maternal random token samples matched with children's tokens.
...
10 samples of the maternal input were created, randomly excluding 1% of word tokens from each stage (1 to 20) until a sample approximately matched children's total number of tokens. 
...
#### Table 2
t test comparing maternal sample and children's total number of tokens (N = 12). The values are the same for each comparison because the random samples have the same total number of tokens (however different word tokens populate the samples; see Table 1 and 3).
